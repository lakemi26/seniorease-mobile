...
export function translateAuthError(error: unknown): string {
  ...
  return code ? errorMessages[code] || defaultMessage : defaultMessage
}

export function translateRecoveryError(error: unknown): string {
  const code = getErrorCode(error)
  return code ? errorMessages[code] || recoveryDefaultMessage : recoveryDefaultMessage
}

export function translateRegistrationError(error: unknown): string {
  const code = getErrorCode(error)
  return code ? errorMessages[code] || registrationDefaultMessage : registrationDefaultMessage
}
