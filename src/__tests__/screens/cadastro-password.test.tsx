...
describe('CadastroScreen password match', () => {
  it('validates password match', async () => {
    const { getByText, findByPlaceholderText, findByLabelText, findByText } = await render(<CadastroScreen />)
    fireEvent.changeText(await findByPlaceholderText('Digite seu nome'), 'Teste')
    fireEvent.changeText(await findByPlaceholderText('Digite seu e-mail'), 'a@b.com')
    fireEvent.changeText(await findByPlaceholderText('Crie uma senha'), 'Senha123')
    fireEvent.changeText(await findByPlaceholderText('Repita a senha'), 'Diferente')
