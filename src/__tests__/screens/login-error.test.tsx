...
describe('LoginScreen auth error', () => {
  it('displays auth error from context', async () => {
    mockAuth({ authError: 'Erro de autenticação' })
    const { findByText } = await render(<LoginScreen />)
    expect(await findByText('Erro de autenticação')).toBeTruthy()
  })
})
